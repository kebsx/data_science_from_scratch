...
# The mean of a Bernoulli(p) variable is p and the std is sqrt(p(1 - p))
# CLT says that as n gets large, Binomial(n, p) variable is approximately a
# normal random variable with mean mu = np and std sigma = sqrt(np(1-p))

from collections import Counter

def binomial_histogram(p: float, n: int, num_points: int, savefig: bool) -> None:
    """Picks points from a Binomial(n, p) and plots their histogarm"""
    ...
    ys = [normal_cdf(i + 0.5, mu, sigma) - normal_cdf(i - 0.5, mu, sigma)
          for i in xs]
    plt.plot(xs, ys)
    plt.title("Binomial Distrubtion vs. Normal Approximation")
    
    if bool:
        plt.savefig("images/binomial_dist_normal_approx.png")
    else:
        plt.show()

    plt.gca().clear()

binomial_histogram(0.75, 100, 10000, False)